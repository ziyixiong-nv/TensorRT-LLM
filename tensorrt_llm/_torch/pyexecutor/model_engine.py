    ...
    def _prepare_tp_inputs(
        ...
        if num_tokens > 0:
            ...
            if self.is_draft_model and num_accepted_tokens_device is not None:
                ...
                # Vectorized to avoid implicit synchronization in the loop
                ...
                # Fully vectorized to avoid implicit synchronization
                if len(first_draft_input_ids_positions) > 0:
                    ...
                    # This is still created on GPU but it's a simple arange operation, not from CPU data
                    token_positions = torch.arange(
                        tokens_per_request, dtype=torch.long,
                        device='cuda').repeat(num_requests)

                    # Single vectorized update - no loops, no sync!
                    self.input_ids_cuda[
                        self.
