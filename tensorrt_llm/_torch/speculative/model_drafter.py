...
class ModelDrafter(Drafter):
    ...
    def process_decoded_tokens(
        ...
        for req in draft_batch.all_requests():
            ...
            print(
                f"[DEBUG] [PROCESS DECODED TOKENS] target_model_req {target_model_req}"
            )
            print(
                f"[DEBUG] [PROCESS DECODED TOKENS] target_model_req.py_draft_tokens: {target_model_req.py_draft_tokens}"
            )
    ...
    def _convert_draft_tensors(
        ...
        new_tokens_lens = torch.ones(scheduled_batch.batch_size, device=device)
        ...
        for idx, request in enumerate(scheduled_batch.generation_requests):
            num_accepted_tokens = request.py_num_accepted_draft_tokens
            new_tokens[0,
                       idx] = new_tensors_device.new_tokens[num_accepted_tokens,
                                                            idx]
        ...
        print(
            f"[DEBUG] [CONVERT DRAFT TENSORS] updated_tensors: {updated_tensors}"
        )
